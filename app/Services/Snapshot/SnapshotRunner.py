...
class SnapshotRunner:
    ...
    @classmethod
    def trigger_perhitungan_stok(cls, server_key, start_date, end_date):
            ...
            t = threading.Thread(
                ...
                args=(server_key, start_date, end_date),
                daemon=True,
            )
    ...
    @classmethod
    def _do_perhitungan_stok(cls, server_key, start_date, end_date):
            from app.Models.Database import db_manager
            ...
            try:
                ...
                FROM (
                    -- Pembelian
                    SELECT t.kd_divisi, d.kd_barang, d.qty AS debet, 0 AS kredit
                    FROM t_pembelian_detail d (NOLOCK)
                    INNER JOIN t_pembelian t (NOLOCK) ON d.no_transaksi = t.no_transaksi
                    ...
                    UNION ALL
                    -- Penjualan
                    SELECT t.kd_divisi, d.kd_barang, 0 AS debet, d.qty AS kredit
                    FROM t_penjualan_detail d (NOLOCK)
                    INNER JOIN t_penjualan t (NOLOCK) ON d.no_transaksi = t.no_transaksi
                    INNER JOIN m_barang b (NOLOCK) ON d.kd_barang = b.kd_barang
                    INNER JOIN m_kategori k (NOLOCK) ON b.kd_kategori = k.kd_kategori
                    ...
                    UNION ALL
                    -- Mutasi Keluar
                    SELECT t.kd_divisi_asal AS kd_divisi, d.kd_barang, 0 AS debet, d.qty AS kredit
                    FROM t_mutasi_stok_detail d (NOLOCK)
                    INNER JOIN t_mutasi_stok t (NOLOCK) ON d.no_transaksi = t.no_transaksi
                    WHERE CAST(t.tanggal AS DATE) BETWEEN CAST(? AS DATE) AND CAST(? AS DATE)
                    ...
                    UNION ALL
                    -- Mutasi Masuk
                    SELECT t.kd_divisi_tujuan AS kd_divisi, d.kd_barang, d.qty AS debet, 0 AS kredit
                    FROM t_mutasi_stok_detail d (NOLOCK)
                    INNER JOIN t_mutasi_stok t (NOLOCK) ON d.no_transaksi = t.no_transaksi
                    WHERE CAST(t.tanggal AS DATE) BETWEEN CAST(? AS DATE) AND CAST(? AS DATE)
                    ...
                    UNION ALL
                    -- Opname Masuk
                    SELECT kd_divisi, kd_barang, qty AS debet, 0 AS kredit
                    FROM t_opname_stok (NOLOCK)
                    WHERE status = 2
                    ...
                    UNION ALL
                    -- Opname Keluar
                    SELECT kd_divisi, kd_barang, 0 AS debet, qty AS kredit
                    FROM t_opname_stok (NOLOCK)
                    WHERE status <> 2
                      AND CAST(tanggal AS DATE) BETWEEN CAST(? AS DATE) AND CAST(? AS DATE)
                ) AS all_trans
